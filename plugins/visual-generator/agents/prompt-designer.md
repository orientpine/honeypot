...
### Content Placement 문장 형식 (MANDATORY)

모든 배치 지시는 **한국어 완전 문장** 구조를 따릅니다. 라벨 나열 형식은 Gemini가 텍스트로 해석하여 렌더링합니다.

**필수 형식**: `{위치}에 {참조}를(을) {방식}으로 배치`

| 문장 성분 | 역할 | 예시 |
|-----------|------|------|
| {위치}에 | 공간 좌표 (조사 `에` 필수) | `좌상단에`, `우하단 영역에`, `Title 1 하단에` |
| {참조}를(을) | CONTENT 번호 참조 (조사 `을/를` 필수) | `Main 1을`, `Title 2를`, `Main 2~5를` |
| {방식}으로 | 시각적 처리 방법 | `대형 볼드로`, `사각 카드 4개로 가로 나열`, `강조색 박스로` |

**올바른 예시**:
```markdown
### Content Placement
- 좌상단에 Title 1을 대형 볼드로 배치
- Title 1 하단에 Title 2를 소제목으로 배치
- 좌하단에 Main 2~5를 사각 카드 4개로 가로 나열
- 우하단에 Main 6을 강조색 박스로 배치
- 시각 요소: 좌상단에서 좌하단으로 보조색 연결선 배치
```

**금지 형식** (`{참조} {위치} {메타라벨}` — 라벨 나열):
```markdown
❌ Main 3 좌하단 보기 카드 1          → 조사 없음, "보기 카드 1" 메타라벨
❌ Main 7 우하단 정답 강조 박스        → 조사 없음, "정답 강조 박스" 메타라벨
❌ Data 4 우하단 협업 정보 태그        → 조사 없음, "협업 정보 태그" 메타라벨
❌ Data 5 Z 흐름 화살표 시작 지점 배치  → 조사 없음, 렌더링 지시문 유출
```

**Content Placement 전용 금지 메타라벨** (위치·방식 표현에서 사용 금지):

| 카테고리 | 금지 표현 |
|----------|-----------|
| **카드 역할명** | 보기 카드, 힌트 카드, 정답 박스, 정답 강조 박스, 해설 영역, 질문 본문 블록 |
| **데이터 역할명** | 데이터 칩, 숫자 블록, 협업 정보 태그, 보조 태그, 수치 블록 |
| **흐름 역할명** | Z 흐름 화살표, 시선 유도 포인트, 연결 노드 |

이들은 `사각 카드`, `강조색 박스`, `소형 태그`, `보조색 연결선` 등 **순수 시각 형태 묘사**로 대체합니다.
...
- [ ] '보기 카드', '정답 박스', '데이터 칩', '힌트 카드' 등 **카드/데이터 역할명이 포함되지 않았는가**?
- [ ] 모든 배치 지시에 **한국어 조사(을/를/에/으로)**가 포함되어 완전 문장인가? (라벨 나열 형식 금지)
...
## Data Elements 작성 원칙 (CRITICAL)

Data Elements는 이미지에 **독립 수치 블록**(숫자 칩, KPI 카드 등)으로 렌더링될 항목만 포함합니다. 구조 설명, 개수 서술, Main Content 중복 값은 절대 포함하지 않습니다.

### 유효한 Data Elements (원자적 수치)

| 유형 | 예시 | 설명 |
|------|------|------|
| 순수 수치 | `956`, `99.2%`, `$2.5M` | Main Content에 동일 수치가 없을 때만 |
| 수치+단위 쌍 | `처리 속도 3.2ms`, `정확도 99.2%` | 짧은 라벨+수치 조합 |
| 날짜/기간 | `2026년`, `3개월` | 독립 시간 블록 |

### 금지되는 Data Elements (메타 설명)

| 금지 패턴 | 문제 | 올바른 처리 |
|-----------|------|------------|
| `보기 4개` | 구조 설명 — 렌더링할 값이 아님 | INSTRUCTION Key Message로 이동 또는 제거 |
| `협업 인원 5` | 서술형 메타라벨 | Main Content에 `AI 에이전트 5명 협업`으로 통합 |
| `정답 선택 ③` | Main Content와 중복 + 메타라벨 | 제거 |
| `항목 3종` | 개수 설명 | 제거 |
| `전체 N단계` | 구조 설명 | 제거 |
| `주요 기술 5가지` | 개수 서술 | 제거 |
...
항목이 다음 질문에 **모두 "예"**이면 Data Elements 적격:

1. 이 텍스트가 이미지에 **독립된 숫자 블록**으로 보여야 하는가?
2. Main Content의 어떤 항목에도 이 값이 **포함되어 있지 않은가**? (부분 포함도 중복)
3. "~개", "~명", "~종", "~단계", "~가지" 같은 개수 접미사를 제거하면 **순수 수치만 남는가**?

하나라도 "아니오"면 → Data Elements에 넣지 않는다. Main Content에 통합하거나 INSTRUCTION의 Key Message로 이동한다.

### 부분 포함 판정 기준

| Main Content 항목 | Data Elements 후보 | 판정 | 이유 |
|-------------------|-------------------|------|------|
| `③ 약 960줄` | `960` | **중복** | 960이 Main 항목에 포함됨 |
| `실제 분량 956줄` | `956` | **중복** | 956이 Main 항목에 포함됨 |
| `정답 ③` | `정답 선택 ③` | **중복** | 동일 의미, 텍스트 거의 동일 |
| `AI 에이전트 5명 협업` | `$2.5M 절감` | **적격** | Main에 없는 독립 수치 |
