...
|------|------|------|
| `좌측 오버레이에 '수동 처리 3시간', '오류율 15%' 텍스트 배치` | `좌측 오버레이: Before 핵심 지표` | `좌측 오버레이에 Data 1–2를 배치` |
| `우측 오버레이에 '자동 처리 10분', '오류율 0.5%' 텍스트 배치` | `우측 오버레이: After 핵심 지표` | `우측 오버레이에 Data 3–4를 배치` |
| `중앙에 '→' 화살표 배치, 하단에 '10배 속도 향상' 텍스트` | `중앙: 변화율 배지` | `중앙에 화살표 배치, 하단에 Main 1을 배치` |
...
- `### Content Placement`: **좌우 장면을 자연어로 상세히 묘사** (환경, 주체, 행동, 톤, 오버레이 처리 포함). 중앙 화살표/VS 배치. CONTENT 항목을 **번호 참조**(`Title N`, `Main N`, `Data N`)로만 참조하여 오버레이 텍스트 배치 위치 설명. **`[Image N]` 플레이스홀더, 텍스트 직접 인용 금지(이중 렌더링 방지), 메타라벨(역할 분류명) 절대 금지**
