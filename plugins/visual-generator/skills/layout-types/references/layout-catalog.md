# Layout Catalog — 24종 레이아웃 상세 정의

> 이 문서는 `/visual-generator:layout-types` 스킬의 상세 레이아웃 정의를 포함합니다.
> 각 레이아웃은 **핵심 아이디어**, **ASCII 시각 구성**, **시각화 원칙**, **권장 사양**, **적합/부적합 케이스**로 구성됩니다.
...
축 기반 레이아웃에서는 **위치가 곧 의미**이므로, 콘텐츠 배치 후 반드시 역검증합니다. X-Y축 혼동으로 사분면 콘텐츠가 뒤바뀌는 오류를 방지합니다.

| 단계 | 행동 |
|------|------|
| 1 | X축 정의와 방향 확인: "{축 이름}" (Low=왼쪽 → High=오른쪽) |
| 2 | Y축 정의와 방향 확인: "{축 이름}" (Low=아래 → High=위) |
| 3 | 각 사분면의 콘텐츠를 읽고, 아래 역검증 질문으로 일치 여부 확인 |
| 4 | 불일치 발견 시 콘텐츠 위치 교정 후 재검증 |
...
```
X축: AI 도구화 역량, Y축: 질문 주도권 수준

좌상 (Low X, High Y): "문제정의 주도권 취약"
→ Y(주도권)=High인데 "주도권 취약"? → ❌ 불일치!

우하 (High X, Low Y): "AI 도구 활용 부족"
→ X(AI 역량)=High인데 "활용 부족"? → ❌ 불일치!

→ 좌상 ↔ 우하 콘텐츠 교정 필요
```

**적용 범위:** Strategy Map뿐 아니라, Contrast(좌/우 의미 구분), Evolution(좌=과거/우=미래), Pyramid(상=우선/하=기반) 등 **위치에 고정 의미가 부여된 모든 레이아웃**에서 이 역검증을 수행합니다.
