...
이 보고서는 국가기관 제출용이므로, 아래 세 가지가 지켜지지 않으면 심사 단계에서 반려됩니다.

**4단계 패턴**: 모든 소섹션(N.1, N.2 등)은 선언 → 문제 정의 → 해결 방안 → 기술적 상세 순서로 작성합니다. 순서 자체가 심사자의 독해 흐름이므로 바꾸지 않습니다.

**구체성**: 추상적 표현 대신 실제 구현체 이름을 씁니다("시스템 개발"이 아니라 "can_bridge_node 개발"). 수치에는 단위를 붙이고(2464mm, 20Hz, 500kbps), 알고리즘은 영문을 병기합니다(게인 스케줄링(Gain Scheduling)). 검증 가능한 기술 문서로 읽히게 하는 요소입니다.

**그림 캡션**: 각 소섹션은 `그림. [구체적 기술 설명]` 형식의 캡션으로 끝냅니다. 후속 도구가 이 형식을 파싱해 도판을 배치합니다.
