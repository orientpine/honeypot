...
분량이 부족한 챕터를 억지로 채우면 보고서 전체의 신뢰도가 떨어지므로, 생성 여부는 자료 충분성 점수로 판단합니다.

- **임계값**: 30점 이상은 생성 대상, 30점 미만은 생략.
- **최소 챕터 보장**: 보고서 형태를 갖추려면 최소 3개 챕터가 필요합니다. 점수 상위 2개 + 결론을 확보하고, 결론(챕터 9)은 점수와 무관하게 항상 생성합니다.
- **번호 재정렬**: 생략된 챕터가 있으면 번호를 순차적으로 다시 매깁니다. 원본 주제명은 그대로 둡니다.
