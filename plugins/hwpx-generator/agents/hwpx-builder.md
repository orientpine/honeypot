...
- **탭/공백 들여쓰기 = 계층**: 글머리 문양(`-`,`*`,`◦`,`□`)이든 번호(`1.`,`a.`,`(1)`,`①`)든, 마크다운 앞 들여쓰기로 단계가 정해진다. `md_parser.py`가 `expandtabs(2)`로 정규화하여 **탭 1번 = 1단계**(공백 2칸 동일). 단계가 깊어지면 문단 전체(줄바꿈 줄 포함)가 우측으로 밀린다. 번호 항목도 글머리와 동일하게 단계가 반영된다 (indent_level 수동 계산 금지 — 파서가 처리).
