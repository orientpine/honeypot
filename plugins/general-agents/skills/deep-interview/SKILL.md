...
4. 폴백은 1회 결정입니다. 같은 도구를 다시 부르면 사용자를 기다리게 만들 뿐 결과는 달라지지 않습니다.
```
...
- **Oversized 처리**: 최신 모델의 큰 컨텍스트 윈도우 덕분에 이 경로는 드물게 발동합니다. 초기 컨텍스트가 실제로 토큰 예산을 넘길 때만, 첫 질문을 요약 요청으로 돌리고 요약이 기록되기 전까지 다음 단계로 넘어가지 않습니다 (블로킹 게이트). 요약 없이 진행하면 이후 라운드가 잘린 컨텍스트 위에서 채점되기 때문입니다.
