...
### 2.2 펀드명 사용 규칙 (CRITICAL)

**펀드 추천 시 반드시 `fund_data.json`의 정확한 펀드명을 사용해야 합니다.**

1. **펀드 선정 후 검증 필수**:
   - 추천할 펀드를 선정한 후, `fund_data.json`에서 해당 펀드의 정확한 `name` 필드 값을 확인
   - 약식 명칭이나 기억에 의존한 명칭 사용 금지

2. **정확한 펀드명 형식 예시**:
   - ✅ 정확: `키움더드림단기채증권투자신탁[채권]C-P2e(퇴직연금)`
   - ❌ 잘못: `키움더드림단기채증권투자신탁(채권)(C-P2e)`
   - ✅ 정확: `한국투자크레딧포커스ESG증권자투자신탁 1(채권)(C-Re)`
   - ❌ 잘못: `한국투자크레딧포커스ESG증권자투자신탁(채권)(C-Re)`

3. **검증 방법**:
   ```bash
   # 펀드명 검색 예시
   powershell -Command "Get-Content 'funds/fund_data.json' -Encoding UTF8 | Select-String '키움더드림'"
   ```

4. **출력 시 주의사항**:
   - 포트폴리오 표, 분석 보고서, JSON 출력 등 모든 곳에서 정식 펀드명 사용
   - compliance-checker가 fund_classification.json에서 찾을 수 있도록 정확한 명칭 필수

### 2.3 출처 필수 (Zero Tolerance)
...
### Step 2.5: material-organizer 결과 확인 (신규)

1. 프롬프트에 material-summary가 포함되었는지 확인
2. 포함됨 → 주제별 정리, 투자 인사이트 추출
   - 사용자가 수집한 자료에서 언급된 섹터/종목 우선 검토
   - 긍정/부정 요인을 포트폴리오 분석에 반영
3. 미포함 → 기존 로직 유지 (material 없이 분석)

**material 입력은 옵셔널**:
- material-summary가 제공되지 않아도 정상 동작
- 제공될 경우 추가 컨텍스트로 활용
